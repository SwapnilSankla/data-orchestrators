...
from airflow_project.plugins.custom_operator.parquet_to_csv_converter import (
    ParquetToCsvOperator,
)
...
def __download_file(aws_conn_id, bucket_name, bucket_key):
    return S3Hook(aws_conn_id).download_file(
        key=bucket_key, bucket_name=bucket_name, local_path="."
    )


def __insert(csv_cleaned_file_path):
    insert_into_postgres_table = PostgresHook(
        postgres_conn_id=os.getenv("POSTGRES_CONN_ID"), schema="airflow"
    )
...
dag = DAG(
    "download_parquet_from_s3_push_to_postgres",
